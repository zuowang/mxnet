...
def _train_multi_device(symbol, ctx, arg_names, param_names, aux_names,
    ...
    handle = pstable.get_table_or_die(0)
    ...
    for i in range(staleness):
        pstable.clock()
    clock = 0
    key = [idx for idx in range(len(executor_manager.param_arrays))]
    val1 = [arg_params[param_names[idx]] for idx in key]
    pstable.table_batch_incx(handle, 1, 1024*1024, key, val1)
    pstable.table_getx(handle, 1, 1024*1024, clock,
                               key, executor_manager.param_arrays)
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                # update parameter
                # push gradient, priority is negative index
                pstable.table_batch_incx(handle, 1, 1024*1024, clock,
                                                 key, executor_manager.grad_arrays)
                # pull back the weights
                pstable.table_getx(handle, 1, 1024*1024,
                                           key, executor_manager.param_arrays)
                clock += 1
...
class FeedForward(BASE_ESTIMATOR):
    ...
    def fit(self, X, y=None, eval_data=None, eval_metric='acc',
            epoch_end_callback=None, batch_end_callback=None, logger=None,
